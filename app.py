...
# Any role that is NOT this value is treated as a firm/admin user with full access.
CLIENT_ROLE = "client"

def is_admin_role(role):
    """Return True for firm/admin users (any role other than 'client')."""
...
def get_user_records():
    """Return list of user dict records from the Users worksheet."""
...
def get_clients_worksheet():
    """Return the Clients worksheet, creating it (with headers) if it doesn't exist."""
    if spreadsheet is None:
        return None
    try:
...
def get_client_records():
    """Return list of client dict records from the Clients worksheet."""
...
def add_client(client_name, contact_name, email, phone):
    ...
    ws.append_row([client_name, contact_name, email, phone, date_added])
    return True
...
# =====================================
    ...
    _added_clients = [str(r.get("Client Name", "")).strip() for r in _client_records if str(r.get("Client Name", "")).strip()]
    ...
    if is_admin:
        ...
        firm_view = st.sidebar.radio(
            "View:",
            ["Practice Dashboard", "Client Workspace"],
            key="firm_view_mode"
        )
        active_client = st.sidebar.selectbox("Manage Client:", CLIENT_LIST)
    else:
        # Client users are LOCKED to their own associated client only.
        ...
        st.sidebar.markdown("<h3 style='text-align: center; margin-top: 20px; font-weight: 400; font-family: Playfair Display, Georgia, serif; color: #333333; letter-spacing: 0.02em;'>Client Account</h3>", unsafe_allow_html=True)
    ...
    st.sidebar.markdown("<br><br><hr style='border-color: #DADDD6; border-width: 1px;'>", unsafe_allow_html=True)
    st.sidebar.markdown(f"<p style='font-size:0.76em; text-align:center; color:#A8B5A3; line-height: 1.6;'>Logged in user:<br><span style='color:#333333; font-weight:600;'>{st.session_state.username}</span></p>", unsafe_allow_html=True)

    # --- MAIN BRAND CONTENT DISPLAY ---
    st.markdown("<h1 class='brand-title'>Clearly Better Books</h1>", unsafe_allow_html=True)
    st.markdown("<p class='brand-tagline'>Bookkeeping that brings clarity, confidence, and calm to your business.</p>", unsafe_allow_html=True)
    st.markdown("<div class='brand-divider'></div>", unsafe_allow_html=True)
    ...
    if task_sheet is not None and invoice_sheet is not None and drive_service is not None:
        ...
        if is_admin and firm_view == "Practice Dashboard":
            ...
            with col1:
                st.markdown(f"""<div class='dashboard-stat'>
                    <span class='dashboard-stat-number'>{len(CLIENT_LIST)}</span>
                    <span class='dashboard-stat-label'>Active Clients</span>
                </div>""", unsafe_allow_html=True)
            with col2:
                st.markdown(f"""<div class='dashboard-stat'>
                    <span class='dashboard-stat-number'>{len(open_tasks_all)}</span>
                    <span class='dashboard-stat-label'>Open Tasks</span>
                </div>""", unsafe_allow_html=True)
            with col3:
                color = "#C4878A" if overdue_tasks else "#333333"
                st.markdown(f"""<div class='dashboard-stat'>
                    <span class='dashboard-stat-number' style='color:{color}'>{len(overdue_tasks)}</span>
                    <span class='dashboard-stat-label'>Overdue Tasks</span>
                </div>""", unsafe_allow_html=True)
            with col4:
                st.markdown(f"""<div class='dashboard-stat'>
                    <span class='dashboard-stat-number'>\${total_ar:,.2f}</span>
                    <span class='dashboard-stat-label'>Total AR Outstanding</span>
                </div>""", unsafe_allow_html=True)

            st.markdown("<hr class='section-divider'>", unsafe_allow_html=True)

            dash_tab1, dash_tab2, dash_tab3 = st.tabs(["All Tasks", "AR Overview", "Client Profiles"])
            ...
            with dash_tab1:
                ...
                with f_col1:
                    filter_client = st.selectbox("Filter by Client", ["All Clients"] + CLIENT_LIST, key="dash_filter_client")
                with f_col2:
                    filter_status = st.selectbox("Filter by Status", ["All", "Pending", "In Progress", "Awaiting Client", "Completed"], key="dash_filter_status")
                with f_col3:
                ...
                if filter_status != "All":
                    tasks_to_show = [t for t in tasks_to_show if str(t.get("status","")).strip() == filter_status]
                if filter_overdue:
                    _filtered = []
                ...
                if not tasks_to_show:
                    ...
                    for idx, task in enumerate(tasks_to_show):
                        ...
                        is_overdue = False
                        try:
                            due_d = datetime.strptime(str(task.get("due","")).strip(), "%Y-%m-%d").date()
                            is_overdue = due_d < today and status != "Completed"
                        except:
                            pass

                        badge = {"Pending": "badge-open", "In Progress": "badge-progress",
                                 "Awaiting Client": "badge-waiting", "Completed": "badge-done"}.get(status, "badge-open")
                        overdue_note = " (overdue)" if is_overdue else ""

                        tc1, tc2, tc3, tc4 = st.columns([3, 2, 2, 2])
                        with tc1:
                            st.markdown(f"**{task.get('task','')}**{overdue_note}")
            ...
            with dash_tab2:
                ...
                for client in CLIENT_LIST:
                    client_unpaid = [i for i in all_invoices if i.get("client") == client and str(i.get("status","")).strip().lower() != "paid"]
                    client_total = 0
                    ...
                    st.markdown(
                        f"<div class='{card_c}'><strong style='font-family:Lato,sans-serif;'>{client}</strong> &nbsp;&nbsp;"
                        f"<span style='color:#A8B5A3;font-size:0.88em;font-family:Lato,sans-serif;'>{len(client_unpaid)} unpaid invoice(s)</span> &nbsp;&nbsp;"
                        f"<strong style='color:{color_ar};'>\${client_total:,.2f} outstanding</strong></div>",
                        unsafe_allow_html=True
                    )
            ...
            with dash_tab3:
                ...
                if st.session_state.show_add_client:
                    with st.form("add_client_form", clear_on_submit=True):
                        ...
                        with ncf1:
                            nc_contact = st.text_input("Contact Name")
                        with ncf2:
                        ...
                        if nc_submit:
                            if not nc_name.strip():
                                ...
                                ok = add_client(nc_name.strip(), nc_contact.strip(), nc_email.strip(), nc_phone.strip())
                                if ok:
                                    st.success(f"Client '{nc_name.strip()}' added. A workspace is now available for them.")
                                    st.session_state.show_add_client = False
                                    st.rerun()
                                else:
                                    st.error("Could not add client. Please check the connection to Google Sheets.")

                st.markdown("<br>", unsafe_allow_html=True)

                for client in CLIENT_LIST:
                    open_tasks = [t for t in all_tasks if t.get("client") == client and str(t.get("status","")).strip() != "Completed"]
                    unpaid_invs = [i for i in all_invoices if i.get("client") == client and str(i.get("status","")).strip().lower() != "paid"]
                    client_ar = 0
                    for inv in unpaid_invs:
                        try:
                            client_ar += float(str(inv.get("amount","")).replace("$","").replace(",","").strip())
                        except:
                            pass
        ...
        # =====================================
            ...
            with tab_tasks:
                ...
                client_tasks = [dict(row, sheet_row=idx+2) for idx, row in enumerate(all_tasks) if row["client"] == active_client]
                ...
                if not client_tasks:
                    ...
                    for task in client_tasks:
                        ...
                        with col_cb:
                            is_done = st.checkbox("Done", key=f"check_{row_num}", value=(curr_status == "Completed"))
                        ...
                        with col_st:
                            if is_admin:
                                ...
                                new_s = st.selectbox("Status", status_opts, index=idx_v,
                                ...
                                badge = {"Pending": "badge-open", "In Progress": "badge-progress",
                                         "Awaiting Client": "badge-waiting", "Completed": "badge-done"}.get(curr_status, "badge-open")
                                st.markdown(f"<span class='status-badge {badge}'>{curr_status}</span>", unsafe_allow_html=True)
                        with col_dt:
                ...
                if is_admin:
                    ...
                    with st.form("new_task_form", clear_on_submit=True):
                        ...
                        with fc1:
                            task_status_new = st.selectbox("Initial Status", ["Pending", "In Progress", "Awaiting Client"])
                        with fc2:
                        ...
                        if submit_button and new_task_text:
                            task_sheet.append_row([active_client, new_task_text, task_status_new, str(due_date)])
            ...
            with tab_invoices:
                ...
                client_invoices = [dict(row, sheet_row=idx+2) for idx, row in enumerate(all_invoices) if row["client"] == active_client]
                unpaid_invoices = [i for i in client_invoices if str(i.get("status","")).strip().lower() != "paid"]
                paid_invoices = [i for i in client_invoices if str(i.get("status","")).strip().lower() == "paid"]

                total_client_ar = 0
                for inv in unpaid_invoices:
                    try:
                        total_client_ar += float(str(inv.get("amount","")).replace("$","").replace(",","").strip())
                    except:
                        pass

                if unpaid_invoices:
                    st.markdown(f"<p style='font-family:Lato,sans-serif; font-size:1.05em; color:#333333; margin-bottom:0;'><strong>Total Outstanding: \${total_client_ar:,.2f}</strong></p>", unsafe_allow_html=True)
                ...
                if not unpaid_invoices:
                    ...
                    for inv in unpaid_invoices:
                                ...
                                <table style="width:100%; border:none; background:none; margin:0; padding:0;">
                                    <tr style="background:none; border:none;">
                                        <td style="border:none; width:45%; padding:0;">
                                            <span style="font-family:'Playfair Display',Georgia,serif; font-size:1.2em; font-weight:600; color:#333333;">Invoice #{inv['invoice_num']}</span><br>
                                            <span style="font-size:0.83rem; color:#A8B5A3; font-family:'Lato',sans-serif; letter-spacing:0.02em;">Due: {inv['due_date']}</span>
                                        </td>
                                        <td style="border:none; width:25%; vertical-align:middle; padding:0;">
                                            <span style="font-size:1.5em; font-weight:600; color:#333333; font-family:'Playfair Display',Georgia,serif;">{inv['amount']}</span>
                                        </td>
                                        <td style="border:none; width:30%; text-align:right; vertical-align:middle; padding:0;">
                        ...
                        if is_admin:
                            if st.button(f"Mark as Paid \u2014 Invoice #{inv['invoice_num']}", key=f"mark_paid_{inv['sheet_row']}"):
                ...
                if is_admin:
                    ...
                    with st.form("new_invoice_form", clear_on_submit=True):
                        ...
                        if submit_inv and inv_num and inv_amt:
                            invoice_sheet.append_row([active_client, inv_num, inv_amt, str(inv_due), inv_url, "Unpaid"])
            ...
            with tab_documents:
                ...
                st.markdown("""
                    <div class="custom-upload-container">
                        ...
                        <div class="custom-upload-subtitle">Supported formats: PDF, PNG, JPG, CSV &nbsp;&middot;&nbsp; Max: 200MB</div>
                    </div>
                """, unsafe_allow_html=True)
